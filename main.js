...
const TIMES_VIEW = document.getElementById('timesView').innerHTML;
...
function render(times) {
    times.sort((a, b) => (a.date + a.startTime < b.date + b.startTime) ? 1 : -1);
...
function onStop() {
    ...
    if (openTime = getOpenTime(times)) {
        openTime.endTime = getTime();
        setTimes(times);
    }
...
function getOpenTime(times) {
    return times.find(time => time.endTime === null);
}
...
function getDurations(times) {
    let durations = CATEGORIES.map(category => ({
        category,
        totalSecs: times
    ...
    return durations.map(({category, totalSecs}) => {
        let hours = Math.floor(totalSecs / 60 / 60).toString().padStart(2, '0');
        let minutes = Math.floor((totalSecs / 60) % 60).toString().padStart(2, '0');
...
let rng = () => crypto.getRandomValues(new Uint8Array(16));
